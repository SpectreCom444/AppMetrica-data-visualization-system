...
def create_vizualization_button():
    ...
    plot_line_button = tk.Button(root, text="Plot Line Chart", command=lambda: plot_line_chart(fig_canvas))
    canvas.create_window(400, 50, window=plot_line_button)

    plot_bar_button = tk.Button(root, text="Plot Bar Chart", command=lambda: plot_bar_chart(fig_canvas))
    canvas.create_window(400, 100, window=plot_bar_button)

    plot_scatter_button = tk.Button(root, text="Plot Scatter Plot", command=lambda: plot_scatter_plot(fig_canvas))
    canvas.create_window(400, 150, window=plot_scatter_button)

    plot_histogram_button = tk.Button(root, text="Plot Histogram", command=lambda: plot_histogram(fig_canvas))
    canvas.create_window(400, 200, window=plot_histogram_button)

    plot_heatmap_button = tk.Button(root, text="Plot Heatmap", command=lambda: plot_heatmap(fig_canvas))
    canvas.create_window(400, 250, window=plot_heatmap_button)

    plot_bubble_button = tk.Button(root, text="Plot Bubble Chart", command=lambda: plot_bubble_chart(fig_canvas))
    canvas.create_window(400, 300, window=plot_bubble_button)

    plot_area_button = tk.Button(root, text="Plot Area Chart", command=lambda: plot_area_chart(fig_canvas))
