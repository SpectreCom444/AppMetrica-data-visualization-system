...
class Plotter:
    ...
    def plot_histogram_chart_split(self, element, event_types, data, x_label):
        if self.histogram_type == HistogramType.COMPARISON:
            ...
            for i, event in enumerate(event_types):
                if self.orientation == Orientation.HORIZONTAL:
                    self.ax.bar([p + i * width for p in positions],
                                data[event], width=width, label=event)
                elif self.orientation == Orientation.VERTICAL:
                    self.ax.barh([p + i * width for p in positions],
            ...
            for event in event_types:
                if self.orientation == Orientation.HORIZONTAL:
                    self.ax.bar(element, data[event],
                                bottom=bottom, label=event)
                elif self.orientation == Orientation.VERTICAL:
                    self.ax.barh(element, data[event],
    ...
    def plot_area_chart_split(self, element, event_types, data, x_label):
        ...
        bottom = [0] * len(element)
        for event in event_types:
            self.ax.fill_between(element, bottom, [
