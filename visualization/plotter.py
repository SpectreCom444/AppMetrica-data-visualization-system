...
class Plotter:
    ...
    def _draw_figure(self):
        self.visualization_config.canvas.ax.set_title(
            self.visualization_config.selected_data)
        self.visualization_config.canvas.figure = self.visualization_config.canvas.fig
    ...
    def plot_line_chart(self, events_count):
        ...
        self.visualization_config.canvas.ax.plot(x, y, marker='o')
        self._draw_figure()
    ...
    def plot_bar_chart(self, events_count):
        ...
        if self.visualization_config.orientation == Orientation.HORIZONTAL:
            self.visualization_config.canvas.ax.bar(x, y)
        elif self.visualization_config.orientation == Orientation.VERTICAL:
            self.visualization_config.canvas.ax.barh(x, width=y)
        self._draw_figure()
    ...
    def plot_pie_chart(self, events_count):
        ...
        x = list(events_count.keys())
        y = list(events_count.values())
        self.visualization_config.canvas.ax.pie(y, labels=x, autopct='%1.1f%%')
        self._draw_figure()
    ...
    def plot_scatter_plot(self, events_count):
        ...
        x = list(events_count.keys())
        y = list(events_count.values())
        self.visualization_config.canvas.ax.scatter(x, y)
        self._draw_figure()
    ...
    def plot_bubble_chart(self, events_count):
        ...
        y = list(events_count.values())
        sizes = y
        self.visualization_config.canvas.ax.scatter(x, y, sizes)
        self._draw_figure()
    ...
    def plot_area_chart(self, events_count):
        ...
        x = list(events_count.keys())
        y = list(events_count.values())
        self.visualization_config.canvas.ax.fill_between(
            x, y, color="skyblue", alpha=0.4)
        self.visualization_config.canvas.ax.plot(
    ...
    def plot_histogram_chart_split(self, element, event_types, data, x_label):
        if self.visualization_config.histogram_type == HistogramType.COMPARISON:
            ...
            for i, event in enumerate(event_types):
                if self.visualization_config.orientation == Orientation.HORIZONTAL:
                    self.visualization_config.canvas.ax.bar([p + i * width for p in positions],
                                                            data[event], width=width, label=event)
                elif self.visualization_config.orientation == Orientation.VERTICAL:
                    self.visualization_config.canvas.ax.barh([p + i * width for p in positions],
                                                             data[event], height=width, label=event)

            if self.visualization_config.orientation == Orientation.HORIZONTAL:
                self.visualization_config.canvas.ax.set_xticks(
                    [p + width * (len(event_types) / 2) - width / 2 for p in positions])
                self.visualization_config.canvas.ax.set_xticklabels(element)
            elif self.visualization_config.orientation == Orientation.VERTICAL:
                self.visualization_config.canvas.ax.set_yticks(
                    [p + width * (len(event_types) / 2) - width / 2 for p in positions])
                self.visualization_config.canvas.ax.set_yticklabels(element)

        elif self.visualization_config.histogram_type == HistogramType.SUMMATION:
            bottom = [0] * len(element)
            for event in event_types:
                if self.visualization_config.orientation == Orientation.HORIZONTAL:
                    self.visualization_config.canvas.ax.bar(element, data[event],
                                                            bottom=bottom, label=event)
                elif self.visualization_config.orientation == Orientation.VERTICAL:
                    self.visualization_config.canvas.ax.barh(element, data[event],
                                                             left=bottom, label=event)
                bottom = [i + j for i, j in zip(bottom, data[event])]

        self.visualization_config.canvas.ax.set_ylabel('Counts')
        self.visualization_config.canvas.ax.set_xlabel(x_label)
        self.visualization_config.canvas.ax.legend()
        self._draw_figure()
    ...
    def plot_scatter_chart_split(self, element, event_types, data, x_label):
        ...
        for event in event_types:
            self.visualization_config.canvas.ax.scatter(
                element, data[event], label=event)
    ...
    def plot_area_chart_split(self, element, event_types, data, x_label):
        ...
        bottom = [0] * len(element)
        for event in event_types:
            self.visualization_config.canvas.ax.fill_between(element, bottom, [
    ...
    def plot_split(self, events_count, x_label, loading):
        try:
            ...
            self.visualization_config.canvas.create_axes()
