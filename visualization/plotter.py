...
class Plotter:
    ...
    def _convert_to_percentage_split_event(self, events_count):
        ...
        for date, events in events_count.items():
            for event in events:
                events[evet] = (events[event] / total) * 100
        return events_count
    ...
    def plot_split(self, events_count, x_label, loading):
        try:
            ...
            if self.visualizatin_config.type_of_measurement == TypeOfMeasurement.PERCENTAGES:
                events_count = slf._convert_to_percentage_split_event(
                    events_count)
